...
import com.quangkhai.vehiceltracking_backend.controller.StationController;
...
public class StationExceptionHandler {
    ...
    public ResponseEntity<ProblemDetail> handleStationNotFound(StationNotFoundException ex, HttpServletRequest request) {
    ...
    public ResponseEntity<ProblemDetail> handleStationConflict(StationConflictException ex, HttpServletRequest request) {
    ...
    public ResponseEntity<ProblemDetail> handleDataIntegrityViolation(DataIntegrityViolationException ex, HttpServletRequest request) {
        ProblemDetail problem = ProblemDetail.forStatusAndDetail(HttpStatus.CONFLICT, "Dữ liệu xung đột hoặc đang được sử dụng trong hệ thống");
        problem.setType(ABOUT_BLANK);
        problem.setTitle("Conflict");
        problem.setInstance(URI.create(request.getRequestURI()));
        return ResponseEntity.status(HttpStatus.CONFLICT).contentType(PROBLEM_JSON).body(problem);
