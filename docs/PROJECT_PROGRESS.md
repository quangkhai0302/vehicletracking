...
> **Feature 018 — Google Routes + HERE Traffic (2026-09-15):** đã triển khai provider Google cho geometry/ETA/traffic intervals, provider pin theo tuyến, CAR/MOTORCYCLE, codec Google/HERE, migration V12, request budget/single-flight, simulator theo duration Google, reroute giữ waypoint và hậu kiểm closure HERE, Google Map Tiles proxy và UI tách màu Google route/HERE area. Reroute chạy theo snapshot → HTTP ngoài transaction → compare-and-write. Backend không-Integration **203 test pass**, frontend type-check/lint đạt; còn chờ PostgreSQL/Docker, Node 24 build, browser/live API và xác nhận retention/billing trước khi bật production. Xem [hồ sơ 018](features/018-google-routes-here-traffic/evidence.md).

> **Sửa lỗi 011 — nhiều tuyến đồng thời (2026-09-15):** bản đồ Mô phỏng nay vẽ tất cả tuyến trong đội, giữ tuyến khác khi chọn xe, màu cyan cho xe chọn, click đoạn chung có danh sách xe; Xem tất cả bao trọn tuyến. Xe mô phỏng đã kết thúc rời layer nhưng giữ lịch sử ở Theo dõi. `SimulationRoutesLayer`, `useSimulationFleet`, `MapComponent`; 10 nhóm kiểm tra hình học/tương tác đa tuyến pass, 9 nhóm đa xe + 7 unit + 12 nhóm traffic regression pass; lint/tsc/build đạt (main 501.00 kB có warning kích thước). GET-only dữ liệu thật: #3 đã hoàn thành, #2 tạm dừng, một tuyến phù hợp đội hiện tại. [Evidence mới](features/011-multi-vehicle-simulator/verification.md).

> **Cập nhật 011 — mô phỏng nhiều xe (2026-09-14):** đã hiện xe có chuyến chờ tại trạm đầu khi mở Mô phỏng; click marker/popup để mở điều khiển riêng, nhiều xe cùng trạm hoặc đang chạy chồng nhau vẫn chọn được. `SimulationFleetLayer`, `SimulationFleetList`, `useSimulationFleet` nối API/SSE hiện có; 7 unit + 9 nhóm browser đa xe đạt, gồm A/B chạy cùng lúc, pause/speed A không đổi B, reload và mobile. Lint/tsc/build đạt. Môi trường thật kiểm tra GET-only có 0 chuyến đủ điều kiện chờ/đang mô phỏng và 1 marker lịch sử, không ghi dữ liệu. Xem [verification 011](features/011-multi-vehicle-simulator/verification.md). Backend không bị giới hạn một xe, nhưng mỗi vehicle chỉ có một chuyến IN_PROGRESS; chưa nghiệm thu tải lớn/HERE nhiều xe thật.

> **Cập nhật 010 — tập trung theo dõi xe (2026-09-14):** theo yêu cầu mới nhất, đã gỡ hover/tải flow cho mọi đường nền. Xe/chuyến được chọn có thẻ vị trí, vận tốc, trạm kế tiếp, ETA và ảnh hưởng giao thông; giữ tooltip tuyến đã chọn. `TripTrafficSummary` dùng telemetry/frame và ETA backend, phân biệt HERE/gần nhất/tuyến đã lưu, xử lý blocked trước fallback; camera giữ marker trong vùng nhìn khi mở thẻ. 9 test chọn dữ liệu ETA, 12 nhóm browser theo dõi xe, 10 nhóm lifecycle simulator hai tab, 12 nhóm hồi quy tooltip và lint/tsc/build đạt. Kiểm tra GET với backend đang chạy: có marker/ETA, chuyến hiện có đã kết thúc (`ROUTE_SNAPSHOT`), không có viewport flow request. Xem [verification 010](features/010-route-traffic-tooltip/verification.md). Hồ sơ mới vẫn local vì `docs/` đang bị ignore.
