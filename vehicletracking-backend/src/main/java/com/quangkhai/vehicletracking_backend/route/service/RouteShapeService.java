...
public class RouteShapeService {
    ...
    public RouteDetailResponse preview(long id,RouteShapeRequest request) {
        return RouteDetailResponse.from(calculate(require(id,false),request,false));
    }
    @Transactional
    public RouteDetailResponse save(long id,RouteShapeRequest request,boolean copy) {
        ...
        var replacement=calculate(source,request,copy);
    ...
    private RouteEntity calculate(RouteEntity source,RouteShapeRequest request,boolean copy) {
        ...
        for (var section:result.sections()) {
            ...
            if (local<2 || local>destinations.size()) throw new ResponseStatusException(BAD_GATEWAY,"HERE trả về chặng không hợp lệ.");
