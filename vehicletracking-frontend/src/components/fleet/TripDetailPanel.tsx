...
export function TripDetailPanel({ detail, loading, busy, error, onClose, onRetry, onAction, onUpdateSchedule, onDeleteTrip, onFocusStop, onSimulate, liveSnapshot }: {
  ...
  const sourceNames: Record<TrafficSource, string> = {
    HERE_LIVE: 'HERE LIVE', HERE_LAST_KNOWN: 'HERE GẦN NHẤT', ROUTE_SNAPSHOT: 'ROUTE SNAPSHOT', UNAVAILABLE: 'KHÔNG KHẢ DỤNG',
  };
  ...
  return <section className="fleet-editor" aria-label="Chi tiết chuyến đi">
    ...
    <div className="fleet-detail-body">
      ...
      {!loading && trip && detail && <>
        ...
        <p className="route-snapshot-note">Lịch kế hoạch của chuyến · Giờ địa phương. Lịch giữ nguyên khi xe khởi hành trễ; check-in ghi nhận xe đi qua vùng trạm, không xác nhận dừng đón/trả khách.</p>
        ...
        <div className="trip-eta-summary" aria-live="polite">
          {eta.loading && !eta.data ? 'Đang tính ETA theo giao thông…' : eta.error ? <>{eta.error} <button className="fleet-text-button" onClick={eta.retry}>Thử lại ETA</button></> : eta.data ? <>
            <strong>ETA theo {sourceNames[eta.data.source]}</strong><span>{eta.data.status === 'BLOCKED' ? 'Có section bị đóng — chưa reroute.' : `Còn khoảng ${Math.ceil(eta.data.totalRemainingSeconds / 60)} phút`}</span>
            {eta.data.trafficObservedAt && <small>Dữ liệu: {displayTripTime(eta.data.trafficObservedAt)} · cập nhật tự động</small>}
        ...
        <ol className="trip-timeline">{detail.stops.map((stop,index) => <li key={stop.sequenceNumber}>
          ...
          <div><button className="fleet-stop-link" onClick={() => onFocusStop([stop.latitude,stop.longitude],16)}><span>{stop.stationName}</span><MapPin size={13} /></button>
            ...
            {etaByStop.get(stop.sequenceNumber)?.state !== 'CHECKED_IN' && <span className="trip-eta-stop">ETA traffic: {etaByStop.get(stop.sequenceNumber)?.etaAt ? displayTripTime(etaByStop.get(stop.sequenceNumber)!.etaAt) : eta.data?.status === 'BLOCKED' ? 'Đường bị đóng' : '—'}</span>}
        ...
        <TelemetryHistoryPanel tripId={trip.id} onFocusPosition={position => onFocusStop(position, 16)} />
...
function VisitStatus({ visit, awaitingExit }: { visit?: StopVisit; awaitingExit?: boolean }) {
  ...
  const source = visit.source === 'SIMULATOR' ? 'GIẢ LẬP' : 'GPS';
  const timeLabel = visit.source === 'SIMULATOR' ? 'Giờ GIẢ LẬP' : 'Giờ GPS';
  const serverTime = visit.source === 'SIMULATOR' ? ` · Giờ máy chủ: ${displayTripTime(visit.actualArrivalAt)}` : '';
  const inferred = visit.evidenceKind !== 'POINT' ? ' · Thời điểm suy ra' : '';
  return <span className="trip-checkin-done">Đã ghi nhận · {source} · {timeLabel}: {displayTripTime(time)}{serverTime}{inferred}</span>;
}
