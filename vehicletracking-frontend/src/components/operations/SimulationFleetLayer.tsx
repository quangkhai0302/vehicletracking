...
export function SimulationFleetLayer({ mapRef, mapReady, visible, vehicles, onSelect }: {
  ...
  const signature = JSON.stringify(vehicles.map(({trip,start}) => ({tripId:trip.id, plate:trip.vehiclePlateNumber,
    routeName:trip.routeName, station:start.stationName, latitude:start.latitude, longitude:start.longitude})));
  useEffect(() => {
    const map = mapRef.current;
    if (!visible || !mapReady || !map) return;
    const rows = JSON.parse(signature) as {tripId:number;plate:string;routeName:string;station:string;latitude:number;longitude:number}[];
    ...
    for (const group of groups.values()) {
      ...
      const marker = L.marker([first.latitude,first.longitude], {keyboard:true,zIndexOffset:900,
        ...
        icon:L.divIcon({className:'simulation-waiting-icon',iconSize:[44,44],iconAnchor:[22,22],
          html:`<div class="simulation-waiting-marker" data-waiting-count="${group.length}"><span aria-hidden="true">🚐</span>${group.length > 1 ? `<b>${group.length}</b>` : ''}</div>`})}).addTo(layer);
      const label = document.createElement('span');
      label.textContent = `${group.length > 1 ? `${group.length} xe` : first.plate} · Chờ xuất phát · 0 km/h · GIẢ LẬP`;
