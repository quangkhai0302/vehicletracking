...
export function AlertStream({ notifications }: Props) {
  ...
  return <section className="alert-stream-content" aria-label="Luồng cảnh báo">
    <div className="source-label"><span className="status-dot" /> HERE Traffic · {unread} chưa đọc <span className="alert-actions"><button className="fleet-text-button" disabled={busy || unread === 0} onClick={() => void readAll()}><CheckCheck size={13} />Đọc tất cả</button></span></div>
